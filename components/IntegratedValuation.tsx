        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">
              <h4 className="font-semibold mb-2">投資判定</h4>
              <div className="text-sm space-y-1">
                {currentMarketCap < consensusRange.min && (
                  <div className="text-green-600 font-medium">
                    ✓ 全評価手法で現在価格を上回る → 強い買い推奨
                  </div>
                )}
                {currentMarketCap >= consensusRange.min && currentMarketCap <= consensusRange.max && (
                  <div className="text-blue-600 font-medium">
                    ◐ 評価レンジ内 → 適正価格帯
                  </div>
                )}
                {currentMarketCap > consensusRange.max && (
                  <div className="text-red-600 font-medium">
                    ✗ 全評価手法で現在価格を下回る → 割高警戒
                  </div>
                )}
                <div className="text-gray-600">
                  現在価格: ¥{(company.stockPrice || 3000).toLocaleString()} / 株
                </div>
                <div className="text-gray-600">
                  価格レンジ: ¥{Math.round(consensusRange.min / company.sharesOutstanding).toLocaleString()} - 
                  ¥{Math.round(consensusRange.max / company.sharesOutstanding).toLocaleString()}
                </div>
              </div>
            </div>
